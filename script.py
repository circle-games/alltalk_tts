...
######################################
base_url = f"http://localhost:{params['port_number']}"
        ...
        with gr.Row():
            with gr.Row():
                ...
                explanation_text = gr.HTML(
                    f"<p>⚙️ <a href='http://localhost:{params['port_number']}'>Settings and Documentation Page</a><a href='http://localhost:{params['port_number']}'></a>⚙️<br>- Low VRAM Mode and Deepspeed take 15 seconds to be enabled or disabled.<br>- The DeepSpeed checkbox is only visible if DeepSpeed is present.</p>"
                )
